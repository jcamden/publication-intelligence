...
Context-derived page numbers are extracted from `page_number` contexts by reading the PDF text layer at the bbox location. This provides an automated way to derive canonical page numbers directly from the document's rendered page numbers.

## Architecture

### 1. Text Extraction Hook (`use-context-derived-page-numbers.ts`)

**Location**: `apps/index-pdf-frontend/src/app/projects/[projectDir]/editor/_hooks/use-context-derived-page-numbers.ts`
...
- Returns `ContextDerivedPageNumber[]` with documentPage, canonicalPage, contextId, contextName
...
- Shows all context-derived page numbers in overview
- Enables bulk rule creation/management

#### Page Pages Content
- Shows context-derived page number for current page
- Allows quick rule creation from context-derived numbers
- Displays source precedence (rule > context > document)

## Answering Your Questions

### Q: Best way to read spans with bounding boxes within page number contexts?
...
1. **Context-derived numbers are extracted** per page:
   ```
   Page 5 → "i"
   Page 6 → "ii"
   Page 7 → "iii"
...
**Design Philosophy**:
1. **Context-derived numbers are authoritative**: They come directly from the PDF
2. **Rules are for corrections/overrides**: Use when context extraction fails or needs adjustment
3. **No automatic rule creation**: Context-derived numbers already work without rules

**UI Workflows**:

#### Option 1: Keep as context-derived (recommended)
...
- User clicks "Create rule from context-derived"
...
**When to create rules from context-derived**:
...
### 1. Multiple page-number contexts

**Scenario**: User creates two overlapping `page_number` contexts

**Behavior**:
- Extract from both contexts
- `computeCanonicalPages` uses **first** context-derived number (by context creation time)
...
- ✅ Re-extract when contexts change

### Manual Testing

**End-to-end workflow**:
1. Create `page_number` context on page 5 (containing "i")
2. Verify "Project Pages" shows: `5 (i, context: Page Numbers)`
3. Verify "Page" section shows: Context-derived: `i`
...
- "Create rules from all contexts" (batch operation)
- Visual diff showing context-derived vs. rules
...
- **No automatic rule creation**: Context-derived numbers work standalone
