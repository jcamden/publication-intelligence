...
Extract text from PDF using PyMuPDF, create TextAtoms in memory, filter by ignore contexts, and prepare for LLM processing. TextAtoms are ephemeral (not persisted) but enable deterministic re-extraction for Stage B mention detection.

## Goals

1. Extract word-level text with bounding boxes from PDF
2. Filter text based on ignore contexts (headers, footers, ignored pages)
...
- Mark TextAtoms `isIndexable=false` if within ignore context bbox
- Don't send non-indexable text to LLM
- Skip ignored pages entirely

**Stage 2 - Post-acceptance:**
- Warn if accepted mention overlaps ignore context
...
type TextAtom = {
  ...
  isIndexable: boolean;    // False if within ignore context
...
export const documentRouter = router({
  ...
  extractText: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      //    - Filter by ignore contexts → mark isIndexable
  ...
  reExtractText: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Re-run extraction (e.g., after ignore contexts changed)
