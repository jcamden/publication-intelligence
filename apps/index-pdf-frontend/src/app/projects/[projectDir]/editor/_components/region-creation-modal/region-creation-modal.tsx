...
export type RegionCreationModalProps = {
	open: boolean;
	onClose: () => void;
	projectId: string;
	currentPage: number;
	...
	regionId?: string; // If provided, edit mode
};

export const RegionCreationModal = ({
	open,
	onClose,
	projectId,
	currentPage,
	...
	drawnBbox,
	regionId,
}: RegionCreationModalProps) => {
	const utils = trpc.useUtils();
	const isEditMode = !!regionId;

	// Fetch region data if editing
	const { data: existingRegion } = trpc.region.list.useQuery(
		{ projectId },
		{
			enabled: isEditMode && !!projectId,
			select: (regions) => regions.find((r) => r.id === regionId),
		},
	);

	const createRegion = trpc.region.create.useMutation({
		onSuccess: () => {
			utils.region.list.invalidate({ projectId });
	...
	const form = useForm({
		defaultValues: {
			name: "",
			regionType: "exclude" as "exclude" | "page_number",
		...
		onSubmit: async ({ value }) => {
			...
			if (isEditMode && regionId) {
				...
				if (!drawnBbox) {
					throw new Error("Bounding box is required for creating region");
				}
	...
	useEffect(() => {
		if (existingRegion && isEditMode) {
			form.setFieldValue("name", existingRegion.name);
			form.setFieldValue("regionType", existingRegion.regionType);
			...
			let formPageConfigMode:
				...
				| "every_other"
				| "custom";
			if (existingRegion.everyOther) {
				formPageConfigMode = "every_other";
			} else if (existingRegion.pageConfigMode === "page_range") {
				// Backend uses "page_range", frontend uses "custom"
				formPageConfigMode = "custom";
			} else {
				formPageConfigMode =
					existingRegion.pageConfigMode as typeof formPageConfigMode;
			}

			form.setFieldValue("pageConfigMode", formPageConfigMode);
			form.setFieldValue("pageRange", existingRegion.pageRange || "");
			form.setFieldValue("startPage", existingRegion.startPage || currentPage);
			form.setFieldValue("endPage", existingRegion.endPage);
			form.setFieldValue(
				"exceptPages",
				existingRegion.exceptPages ? existingRegion.exceptPages.join(", ") : "",
			);
		}
	}, [existingRegion, isEditMode, form, currentPage]);
	...
	return (
		<Modal
			open={open}
			onClose={handleClose}
			title={isEditMode ? "Edit Region" : "Create Region"}
			size="lg"
			footer={
				<>
					...
					{(field) => (
						<Field>
							...
							<Select
								value={field.state.value}
								onValueChange={(value) =>
									field.handleChange(value as "exclude" | "page_number")
								}
							>
								<SelectTrigger>
									<SelectValue />
							...
							<form.Field
								...
								validators={{
									onSubmit: ({ value }) => {
										...
										const pages = value.split(",").map((p: string) => p.trim());
