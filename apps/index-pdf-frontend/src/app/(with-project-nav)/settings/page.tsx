...
import { mentionCreationShowPageSublocationAtom } from "@/app/projects/[projectDir]/editor/_atoms/pdf-chrome-atoms";
											...
											{paidModels.length > 0 && (
												<>
													...
													{paidModels.slice(0, 20).map((model) => (
														<SelectItem key={model.id} value={model.id}>
															{model.name} ($
															{Number.parseFloat(model.pricing.prompt).toFixed(
																6,
															)}
															/1k tokens)
														</SelectItem>
				...
				<CardContent>
					<div className="flex items-start gap-3">
						...
						<div className="grid gap-1.5 leading-none">
							...
							<p className="text-sm text-muted-foreground">
								When creating a mention (subject index), show the optional page
								sublocation field. You can always edit it later in the mention
								details popover.
							</p>
						</div>
					</div>
